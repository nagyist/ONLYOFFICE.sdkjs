...
CDocumentContent.prototype.CheckFormViewWindowPDF = function()
{
	var isChanged = false;

	var oParagraph  = this.GetElement(0);
	var oPageBounds = this.GetContentBounds(0);
	var oFormBounds = this.ParentPDF.getFormRelRect();
	...
	if (oPageBounds.Right - oPageBounds.Left > oFormBounds.W)
	{
		if (oCursorPos.X < oFormBounds.X + nPad)
			nDx = oFormBounds.X + nPad - oCursorPos.X;
		else if (oCursorPos.X > oFormBounds.X + oFormBounds.W - nPad)
			nDx = oFormBounds.X + oFormBounds.W - nPad - oCursorPos.X;
	}
